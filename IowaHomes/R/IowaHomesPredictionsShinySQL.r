...
#'@description This function will connect to the SQL database in order to query the information stored. The function will long and get the required data, after this it will create a dataframe with the chosen columns for the prediction.
...
#'@description This function includes everything needed to make a prediction from a set of data. It uses the kknn method to get a model then after creating a data frame we use predict() with our model and dataframe to get our prediction. The function then rounds the output.
